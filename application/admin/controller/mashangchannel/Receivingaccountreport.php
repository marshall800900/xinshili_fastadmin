 ...
 * @icon fa fa-circle-o
 */
class Receivingaccountreport extends Backend
{

    /**
     * Receivingaccountreport模型对象
     * @var \app\admin\model\mashangchannel\Receivingaccountreport
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\mashangchannel\Receivingaccountreport;
    ...
    public function index()
    {
        ...
        $list = $this->model
            ->field('
                date_key, receiving_account_code,receiving_account_id, admin_id, 
                sum(create_order_number) create_order_number,
                sum(create_order_amount) create_order_amount,
                sum(success_order_number) success_order_number,
                sum(success_order_amount) success_order_amount,
            ...
            ->where($where_extra)
            ->order($sort, $order)
            ->group('date_key,receiving_account_id, receiving_account_code, admin_id')
            ->paginate($limit)
            ->each(function ($value) {
