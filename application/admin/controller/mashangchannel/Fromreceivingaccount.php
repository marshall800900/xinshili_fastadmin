 ...
 * @icon fa fa-circle-o
 */
class Fromreceivingaccount extends Backend
{
    ...
    public function index()
    {
        ...
        if (session('admin.type') != AdminHelper::ADMIN) {
            $where_extra = [
                'admin_id' => ['in', db()->name('admin')->where('line', 'like', ',%'.session('admin.id').'%,')->column('id')]
            ];
        }
    ...
    public function checkOnline($ids = null){
        try{
            $info = db()->name('receiving_account')
                ->field('receiving_account_code, cookie, proxy_ip, proxy_ip_invalid_time, id,area')
                ->where('id', $ids)
                ->find();
            if (!$info)
