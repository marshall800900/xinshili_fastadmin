 ...
 * @icon fa fa-circle-o
 */
class Paychannelreport extends Backend
{

    /**
     * Paychannelreport模型对象
     * @var \app\admin\model\report\Paychannelreport
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\report\Paychannelreport;
    ...
    public function index()
    {
        ...
        $list = $this->model
            ->field('
                date_key,pay_channel_id, device, 
                sum(create_order_number) create_order_number,
                sum(create_order_amount) create_order_amount,
                sum(success_create_order_number) success_create_order_number,
                sum(success_create_order_amount) success_create_order_amount,
            ...
            ')
            ->where($where)
            ->group('date_key,pay_channel_id, device')
            ->order($sort, $order)
            ->paginate($limit)
            ->each(function ($value) {
                $value['pay_channel_id'] = db()->name('pay_channel')
                    ...
                    ->value('name');
                $value['success_rate'] = number_format($value['success_order_number'] / ($value['create_order_number'] ? $value['create_order_number'] : 1) * 100, 2, '.', '');
                $value['success_create_rate'] = number_format(100-$value['success_create_order_number'] / ($value['create_order_number'] ? $value['create_order_number'] : 1) * 100, 2, '.', '');
            });
