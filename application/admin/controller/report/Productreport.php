...
class Productreport extends Backend
{
    ...
    /**
     * Productreport模型对象
     * @var \app\admin\model\report\Productreport
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\report\Productreport;
    ...
    public function index()
    {
        ...
        $list = $this->model
            ...
            ')
            ->where($where)
            ->group('date_key,product_code, device')
            ->order($sort, $order)
            ->paginate($limit)
            ->each(function ($value) {
